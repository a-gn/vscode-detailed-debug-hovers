...
The extension uses GitHub Actions for continuous integration and automated deployment.

### Testing Workflow (`test.yml`)

Runs automatically on:
- **Pull requests to main branch** - Ensures all PRs pass tests before merging
- **Called by deployment workflow** - Runs tests before publishing to marketplace
...
**Important**: All tests must pass before deployment. The deployment workflow automatically runs the test workflow first.
...
- **GitHub Actions**:
  - `.github/workflows/test.yml` - CI testing on PRs and deployment
  - `.github/workflows/publish.yml` - Automated deployment on tag push
